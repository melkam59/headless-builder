      ...
      <div className="max-w-7xl mx-auto">
        <div className={`grid md:grid-cols-2 gap-8 items-center ${!isImageLeft ? 'md:flex-row-reverse' : ''}`}>
          ...
          <div className={`${!isImageLeft ? 'md:order-2' : ''}`}>
            <div className={`${ratioClasses[imageRatio as keyof typeof ratioClasses]} overflow-hidden rounded-lg`}>
              <img
                src={image}
                alt="Feature"
...
function NestedBlockRenderer({ block }: { block: Block }) {
  if (block.type === 'heading') {
    ...
    return (
      <h2 className={`${sizeClasses[size as keyof typeof sizeClasses]} font-bold mb-4 text-gray-900`}>
        {text}
      </h2>
    );
  }
